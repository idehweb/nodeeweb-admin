      ...
      <ArrayInput source="values" label={translate("resources.attributes.values")}>
        <SimpleFormIterator {...props}>
          <FormDataConsumer>
            ...
            return ([<TextInput key={0} source={getSource("name.fa")} label={translate("resources.attributes.name")} multiline fullWidth/>,
              <TextInput
                key={1}
                ...
                multiline
                fullWidth/>,
              <div key={2} className={'posrel'}>{formData['type']==='color' && <ColorPicker
                className={"input-color"}
                color={color}
                source={getSource("color")}
                onChangeComplete={(e) => {console.log("scopedFormData",p,e);
