...
const EntryFormChart = (props) => {
  ...
  const lastMonthDays = Array.from({ length: defaultDays > 0 ? defaultDays : 30 }, (_, i) => subDays(lastDay, i));
  ...
  const getRevenuePerDateFilter = (entriessss,dateCallBack,type,days = 0) => {
    let arrayFilter = [];
